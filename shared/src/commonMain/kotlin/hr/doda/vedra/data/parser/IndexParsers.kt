    ...
    fun parse(xml: String): UvIndexSnapshot {
        ...
        val stations =
            rows.drop(1).mapNotNull { row ->
                ...
                val readings =
                    row.children("Termin").mapIndexedNotNull { idx, t ->
...
object BioForecastParser {
    fun parse(xml: String): BioForecast {
        val root = parseXml(xml)
        val days =
            root.children("Podaci").map { day ->
                BioForecastDay(
                    date =
                        parseDmyDate(day.textOf("Datum"))
...
object HeatColdWaveParser {
    fun parseHeat(xml: String): HeatColdWaveSnapshot = parse(xml, HeatColdWaveSnapshot.Kind.HEAT)

    fun parseCold(xml: String): HeatColdWaveSnapshot = parse(xml, HeatColdWaveSnapshot.Kind.COLD)

    private fun parse(
        xml: String,
        kind: HeatColdWaveSnapshot.Kind,
    ): HeatColdWaveSnapshot {
        val root = parseXml(xml)
        val section = root.child("section") ?: error("Missing <section>")
        val cities =
            section.children("station").map { st ->
                val params =
                    st
                        .children("param")
