...
object MeteoAlertParser {
    fun parse(
        xml: String,
        target: MeteoAlertSet.Target = MeteoAlertSet.Target.TODAY,
    ): MeteoAlertSet {
    ...
    private fun parseInfo(info: XmlNode): MeteoAlertInfo {
        ...
        return MeteoAlertInfo(
            ...
            severity =
                AlertSeverity
                    .fromAwarenessLevel(params["awareness_level"])
    ...
    private fun parseInstant(raw: String?): Instant? = cleanText(raw)?.let { runCatching { Instant.parse(it) }.getOrNull() }
}
...
object HydroBulletinParser {
    fun parse(xml: String): HydroBulletin {
        val root = parseXml(xml)
        val rivers =
            listOf("sava", "kupa", "dunav", "mura", "drava", "neretva")
                .mapNotNull { tag ->
